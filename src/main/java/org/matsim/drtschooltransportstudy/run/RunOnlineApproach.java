...
import org.matsim.drtschooltransportstudy.analysis.SchoolTripsAnalysis;
import org.matsim.drtschooltransportstudy.run.dummyTraffic.DvrpBenchmarkTravelTimeModuleFixedTT;
import org.matsim.drtschooltransportstudy.run.rebalancing.RuralScenarioRebalancingTCModule;
import org.matsim.drtschooltransportstudy.run.routingModule.SchoolTrafficRouteCreator;
...
public class RunOnlineApproach implements MATSimAppCommand {
	...
	private Logger logger = LogManager.getLogger(RunOnlineApproach.class);
	...
	public Integer call() throws Exception {
		...
		while (fleetSize <= maxFleetSize) {
			...
			// By default, the first drt config group is the one we are using
					...
					controler.addOverridingModule(new AbstractDvrpModeModule(drtCfg.getMode()) {
						...
						public void install() {
							bindModal(DefaultMainLegRouter.RouteCreator.class).toProvider(
									// not singleton
									new SchoolTrafficRouteCreator.SchoolTripsDrtRouteCreatorProvider(drtCfg));
						}
					});
				}
			}
			...
			if (onTimeRate >= 0.99 && numServedRequests == numRequests) {
				// max 2 more runs and then stop
				maxFleetSize = Math.min(maxFleetSize, fleetSize + stepSize * 2);
			}
			fleetSize += stepSize;
		}
