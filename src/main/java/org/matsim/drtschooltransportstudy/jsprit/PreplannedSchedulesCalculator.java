...
import org.matsim.drtschooltransportstudy.run.CaseStudyTool;
...
/**
 * Calculates a preplanned schedule for drt school trips.
 * @author Michal Maciejewski (michalm)
 */
public final class PreplannedSchedulesCalculator {
	...
	/**
	 * Calculates the drt task schedule.
	 * */
	...
	public PreplannedSchedules calculate() {
		...
		for (Person person : population.getPersons().values()) {
			...
			// For this study, there is only 1 trip per student
			assert trips.size() == 1;
	...
	/**
	 * Class to hold general settings for the schedule calculation.
	 */
	...
	private static class SchoolTrafficObjectiveFunction implements SolutionCostCalculator {
		...
		Logger logger = LogManager.getLogger(SchoolTrafficObjectiveFunction.class);
		// Most important objective
		private final double unassignedPenalty = 10000;
		// Second most important objective (--> 0 when finite fleet is used)
		private final double costPerVehicle = 200;
		// When possible, passengers should arrive as early as possible to maximize on-time arrival rate
		private final double arrivalTimeCostPerHour = 18.0;
		// Less important objective
		private final double drivingCostPerHour = 6.0;
		...
		public double getCosts(VehicleRoutingProblemSolution solution) {
			...
			if (!options.infiniteFleet) {
				// (when finite fleet is used, we can use all the vehicle as they are already there)
				costForFleet = 0;
			}

			VehicleRoutingTransportCosts costMatrix = problem.getTransportCosts();
			double totalTransportCost = 0;
			// The absolute value doesn't matter. We just want to minimize the sum arrival time (i.e., arrive early)
			...
			// In current setup, transport cost = driving time
			totalTransportCost = totalTransportCost / 3600 * drivingCostPerHour;
			...
			if (options.printProgressStatistics) {
				logger.info("Number of unassigned jobs: {}", numUnassignedJobs);
				logger.info("Number of vehicles used: {}", numVehiclesUsed);
				logger.info("Transport cost of the whole fleet: {}", totalTransportCost);
