...
//                            if (((Activity) pE).getType().startsWith("educ_")) {
	...
	private void modifyHomeActivity(Activity homeActivity, List<Geometry> residentialAreas) {
		...
		if (residentialAreas.size() == 0) {
			...
			while (!locationFound) {
				Coord diffusedCoord = diffuseCoord(originalCoord);
				if (counter >= 200) {
					log.warn("No suitable coordinate found in the residential area. Will simply use the diffused coord");
					homeActivity.setCoord(diffusedCoord);
					break;
				}
	...
	private void modifyEducationActivity(Activity identifiedEducationActivity, Map<String, List<Coord>> schoolData, int age) {
		//13:00:00 This value doesn't really matter. Just set them to the same value in order to remove some strange values from raw data
		identifiedEducationActivity.setEndTime(46800);
		...
		if (minDistance >= 1500) {
			log.warn("Warning: distance adjustment over 1500m : {}", minDistance);
		}
	...
	private Map<String, List<Coord>> readSchoolFacility(ActivityFacilities activityFacilities) {
		...
		// For unknown activity types (age between 12 - 18 --> map to Secondary or Tertiary)
		schoolData.put(UNKNOWN, new ArrayList<>());
		...
		for (ActivityFacility facility : activityFacilities.getFacilities().values()) {
			...
			if (schoolName.contains("Grund")) {
				...
				// will be used for children with age 12 - 18, with unknown activity type
				schoolData.get(UNKNOWN).add(facility.getCoord());
			}
			...
			if (!facilityAssigned) {
				// The rest of the educational facilities belongs to Tertiary
				schoolData.get(EDUC_TERTIARY).add(facility.getCoord());
			}
		}
		return schoolData;
	}
