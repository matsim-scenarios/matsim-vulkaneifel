...
public class PrepareDrtStops implements MATSimAppCommand {
	...
	public Integer call() throws Exception {
		...
		for (Person person : plans.getPersons().values()) {
			// In this project, there is 1 school trip per student
			TripStructureUtils.Trip trip = TripStructureUtils.getTrips(person.getSelectedPlan()).get(0);
			Link toLink = NetworkUtils.getNearestLink(network, trip.getDestinationActivity().getCoord());
			// add DRT stop for school (if not yet in the set)
			stopLinksToWrite.add(toLink);
