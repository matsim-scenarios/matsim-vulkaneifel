...
public class CreatePTFromGtfs implements MATSimAppCommand {
    ...
    public static void main(String[] args) {
        System.exit(new CommandLine(new CreatePTFromGtfs()).execute(args));
    }
    ...
    private static Scenario getScenarioWithPseudoPtNetworkAndTransitVehicles(Network network, TransitSchedule schedule) {
        ...
        VehicleType ptVehicleType = vehicleFactory.createVehicleType( Id.create( "Pt_veh_type", VehicleType.class ) );
        {
            ...
            VehicleUtils.setAccessTime(ptVehicleType, 1.0); // 1s per boarding agent, distributed on 1 door
            VehicleUtils.setEgressTime(ptVehicleType, 1.0); // 1s per alighting agent, distributed on 1 door
