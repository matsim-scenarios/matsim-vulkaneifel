...
Prepare-Runs_Make/temp/vulkaneifel-pt-bus-schedule-without-SEV-line.xml.gz: Prepare-Runs_Make/temp/vulkaneifel-pt-bus-schedule.xml.gz
	java -Xmx80G -jar matsim-vulkaneifel-1.0-SNAPSHOT.jar prepare remove-bus-line\
		--schedule $(BUS)-transitSchedule.xml.gz\
		--name $(BUS_EDIT)\
		--output $(VERSION)/temp
...
Prepare-Runs_Make/temp/vulkaneifel-pt-regional-train-scheduletrain-schedule.xml.gz: #Prepare-Runs_Make/temp/vulkaneifel-pt-big-bus-schedule.xml.gz
	java -Djava.io.tmpdir=${TMPDIR} -Xmx80G -jar matsim-vulkaneifel-1.0-SNAPSHOT.jar prepare create-train-line	\
		--network $(VERSION)/temp/vulkaneifel-network-with-pt.xml.gz	\
		--schedule $(VERSION)/temp/$(BIGBUS)-transitSchedule.xml.gz	\
		--vehicles $(VERSION)/temp/$(BIGBUS)-transitVehicles.xml.gz	\
		--shp dilutionArea/dilutionArea.shp\
		--shp-crs EPSG:25832\
        --target-crs EPSG:25832\
		--name vulkaneifel-regional-train\
		--output $(VERSION)/temp

#create train schedule
Prepare-Runs_Make/temp/vulkaneifel-pt-train-schedule.xml.gz: #Prepare-Runs_Make/temp/vulkaneifel-network.xml.gz
	java -Djava.io.tmpdir=${TMPDIR} -Xmx80G -jar matsim-vulkaneifel-1.0-SNAPSHOT.jar prepare pt-from-gtfs\
...
Prepare-Runs_Make/input/vulkaneifel-complete-schedule.xml.gz: Prepare-Runs_Make/temp/vulkaneifel-pt-train-schedule.xml.gz Prepare-Runs_Make/temp/vulkaneifel-pt-regional-train-scheduletrain-schedule.xml.gz Prepare-Runs_Make/temp/vulkaneifel-pt-bus-schedule-without-SEV-line.xml.gz
	java -Djava.io.tmpdir=${TMPDIR} -Xmx80G -jar matsim-vulkaneifel-1.0-SNAPSHOT.jar prepare merge-transit-schedules\
        ...
        $(VERSION)/temp/$(BUS_EDIT)-transitSchedule.xml.gz\
		$(VERSION)/temp/vulkaneifel-regionl-train-transitSchedule.xml.gz\
		--vehicles $(VERSION)/temp/$(BUS_EDIT)-transitVehicles.xml.gz\
        $(VERSION)/temp/$(TRAIN)-transitVehicles.xml.gz\
		$(VERSION)/temp/vulkaneifel-regional-train-transitVehicles.xml.gz\
		--name vulkaneifel\
		--network $(VERSION)/temp/vulkaneifel-network.xml.gz\
		--output $(VERSION)\
